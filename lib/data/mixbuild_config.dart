...
MixbuildProjectType _parseProjectType(String value) {
  ...
  return MixbuildProjectType.values.firstWhere(
    (item) => item.name == normalizedValue,
...
class MixbuildConfig {
  ...
  factory MixbuildConfig.fromYaml({
    ...
    return MixbuildConfig(
      ...
      workspace: MixbuildWorkspaceConfig(
        ...
        rootPath: _asString(
          workspaceMap['root_path'],
          field: 'workspace.root_path',
        ),
      ),
      ...
      dependencies: dependencyList
          .map((entry) {
            ...
            return MixbuildRepoConfig(
              name: _asProjectName(item['name'], field: 'dependencies[].name'),
      ...
      buildScenarios: scenarioList
          ...
          .map((entry) {
            ...
            return MixbuildScenarioConfig(
              ...
              dependencyOverrides: _parseDependencyOverrides(
                item['dependency_overrides'],
              ),
            );
          })
          .toList(growable: false),
  ...
  static String workspaceSlugFor(String workspaceName) =>
      _slugify(workspaceName);

  String toYamlString() {
    _validateProjectName(mainProject.name, field: 'main_project.name');
    for (final dependency in dependencies) {
      _validateProjectName(dependency.name, field: 'dependencies[].name');
    }
...
String _asProjectName(Object? value, {required String field}) {
  return _validateProjectName(_asString(value, field: field), field: field);
}
