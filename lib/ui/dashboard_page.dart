...
class _DashboardPageState extends ConsumerState<DashboardPage> {
	...
	Future<void> _openConfigCenter() async {
		...
		final result = await showDialog<GlobalConfig>(
			context: context,
			barrierDismissible: true,
			builder: (context) => _ConfigCenterDialog(config: dashboardState.globalConfig),
		);
		if (result != null) {
	...
	Future<void> _openAddScenarioDialog() async {
		...
		final result = await showDialog<BuildScenario>(
			...
			barrierDismissible: true,
			builder: (context) =>
					_AddScenarioDialog(baseDependencies: dashboardState.selectedScenario.dependencies),
		);
		if (result == null) {
			return;
		}
						...
						child: Column(
							children: [
								_TopBar(
									...
									onWorkspaceChanged: (value) => _controller.switchWorkspace(value),
									...
									child: Row(
										children: [
											_SideBar(
												...
												cleanBeforeBuild:
														dashboardState.cleanBeforeBuild[dashboardState.selectedScenarioId] ?? false,
												...
												onTrigger: () => _toggleScenario(selectedProject, selectedScenario),
																...
																child: isWide
																	? Row(
																		...
																		children: [
																			Expanded(
																				flex: 7,
																				child: _buildMatrix(theme, dashboardState),
																			),
																			...
																			Expanded(
																				...
																				child: _InspectorPanel(
																					project: selectedProject,
																					scenario: selectedScenario,
																					onOpenYaml: () => _openYamlDialog(selectedScenario),
																				),
																			),
																		],
																	)
			...
			child: SingleChildScrollView(
				...
				child: Column(
					...
					children: [
						...
						const SizedBox(height: 4),
						Text('v3.1.0-stable · Flutter Desktop / macOS', style: theme.textTheme.bodySmall),
						const SizedBox(height: 20),
						Container(
							padding: const EdgeInsets.all(16),
							decoration: MixBuildTheme.glassPanel(radius: 20, color: MixBuildPalette.surface),
							child: Column(
								crossAxisAlignment: CrossAxisAlignment.start,
								children: [
									Row(
									...
									Text('主工程分支', style: theme.textTheme.labelLarge?.copyWith(color: MixBuildPalette.muted)),
						...
						const SizedBox(height: 16),
						Text('依赖项拓扑视图', style: theme.textTheme.labelLarge?.copyWith(color: MixBuildPalette.muted)),
								...
								child: Container(
									...
									child: Column(
										...
										children: [
											Row(
												children: [
													Icon(dependency.icon, size: 16, color: dependency.highlight ?? MixBuildPalette.muted),
													const SizedBox(width: 8),
													Expanded(child: Text(dependency.name, style: theme.textTheme.titleMedium)),
													if (dependency.isOverride)
														Container(
															padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
															decoration: BoxDecoration(
																color: (dependency.highlight ?? MixBuildPalette.primary).withValues(alpha: 0.12),
																borderRadius: BorderRadius.circular(999),
															),
															child: Text('Override', style: theme.textTheme.bodySmall?.copyWith(color: dependency.highlight ?? MixBuildPalette.primary)),
														),
												],
											),
											const SizedBox(height: 10),
						...
						const SizedBox(height: 16),
						Text('构建场景配置', style: theme.textTheme.labelLarge?.copyWith(color: MixBuildPalette.muted)),
						const SizedBox(height: 10),
						DropdownButtonFormField<String>(
						...
						FilledButton.icon(
							...
							icon: Icon(scenario.status.controlsLocked ? Icons.sync : Icons.play_arrow),
							label: Text(scenario.status.triggerLabel),
						),
						const SizedBox(height: 10),
						OutlinedButton.icon(
							onPressed: scenario.status.canStop ? onStop : null,
							style: OutlinedButton.styleFrom(foregroundColor: MixBuildPalette.error),
														...
														child: ListView(
															...
															children: [
																...
																Row(
																	children: [
																		Expanded(
																			child: TextField(controller: _workspaceController),
																		),
																		const SizedBox(width: 12),
																		OutlinedButton.icon(
																			onPressed: () {},
																...
																GridView.builder(
																	...
																	itemBuilder: (context, index) {
																		final binding = widget.config.bindings[index];
																		return Container(
																			...
																			child: Column(
																				...
																				children: [
																					...
																					Expanded(
																						child: TextField(
																							controller: _bindingControllers[index],
																						),
																					),
																				],
																			),
																	...
																	onPressed: () {
																		Navigator.of(context).pop(
																			widget.config.copyWith(
																				...
																				activeProjectName: _projectNameController.text,
																				bindings: [
																					for (var i = 0;
																						...
																						WorkspaceBinding(
																							projectName:
																									widget.config.bindings[i].projectName,
...
class _AddScenarioDialogState extends State<_AddScenarioDialog> {
	...
	void initState() {
		...
		_commandController = TextEditingController(text: './gradlew assembleRelease');
		_outputController = TextEditingController(text: 'output_dir/');
		_tagController = TextEditingController(text: 'release_');
		_branches = {for (final item in widget.baseDependencies) item.name: item.branch};
	}
